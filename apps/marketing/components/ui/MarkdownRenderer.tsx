...
export default function MarkdownRenderer({ content, className = '' }: MarkdownRendererProps) {
    const html = useMemo(() => {
        ...
        let result = content
            ...
            .replace(/>/g, '&gt;')
            // Headers
            .replace(/^### (.+)$/gm, '<h3 class="text-base font-medium text-slate-800 mt-4 mb-2">$1</h3>')
            ...
            // Bold and Italic
            .replace(/\*\*\*(.+?)\*\*\*/g, '<strong class="font-semibold text-slate-800"><em>$1</em></strong>')
        ...
        if (!result.startsWith('<h') && !result.startsWith('<li')) {
            result = `<p class="mb-3 text-slate-600 leading-relaxed">${result}</p>`;
        }
