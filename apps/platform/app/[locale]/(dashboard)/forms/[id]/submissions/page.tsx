...
const STATUS_STYLES: Record<string, string> = {
    ...
    // Public lead statuses
    PUBLIC_SENT: 'bg-brand/10 text-brand',
    PUBLIC_OPENED: 'bg-brand/20 text-brand',
    PUBLIC_COMPLETED: 'bg-brand/30 text-brand',
    PUBLIC_EXPIRED: 'bg-muted text-foreground/40',
};
...
export default function SubmissionsPage() {
    ...
    async function loadData() {
        ...
        try {
            ...
            // Get unified submissions (assignments + public leads)
    ...
    // Helper to determine if submission is from public lead
    const isPublicLead = (status: string) => status.startsWith('PUBLIC_');

    // Get display status (remove PUBLIC_ prefix)
    const getDisplayStatus = (status: string) => {
        return status.replace('PUBLIC_', '');
    };
                        ...
                        <table className="w-full">
                            <thead className="bg-muted border-b border-border">
                                <tr>
                                    ...
                                    <th className="text-left py-3 px-4 text-sm font-medium text-foreground/70">Source</th>
                            ...
                            <tbody>
                                {assignments.map((assignment) => {
                                    const isPublic = isPublicLead(assignment.status);
                                    const displayStatus = getDisplayStatus(assignment.status);
                                        ...
                                        >
                                            ...
                                            <td className="py-3 px-4">
                                                {isPublic ? (
                                                    ...
                                                    <span className="text-xs px-2 py-1 rounded bg-muted text-foreground/60">
                                                        👤 Patient
                                                    </span>
                                                )}
                                            </td>
                                            ...
                                            <td className="py-3 px-4 text-right">
                                                <Link
                                                    href={isPublic ? `/leads` : `/patients/${assignment.patient_id}`}
                                                    className="text-foreground/70 hover:text-foreground text-sm"
                                                >
                                                    {isPublic ? 'View Lead →' : 'View Patient →'}
                                                </Link>
                                            </td>
                                        </tr>
                                    );
