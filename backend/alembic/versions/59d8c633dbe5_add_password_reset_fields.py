...
def upgrade() -> None:
    ...
    # Create enum type using raw SQL to truly control IF NOT EXISTS
    op.execute("""
        DO $$ BEGIN
            CREATE TYPE datasettype AS ENUM ('CLINICAL_NOTE', 'TRANSCRIPT', 'CHAT_ANALYSIS');
        EXCEPTION
    ...
    op.alter_column(
        "availability_blocks", "schedule_id", existing_type=sa.UUID(), nullable=False
    )
    op.alter_column(
...
def downgrade() -> None:
    ...
    op.alter_column("ai_usage_logs", "user_id", existing_type=sa.UUID(), nullable=False)
